...
export default function AuthPage() {
    ...
    <div className="flex items-center justify-center min-h-screen w-full relative overflow-hidden p-10">
      ...
      <div className="flex w-full max-w-5xl backdrop-blur-md bg-white bg-opacity-20 shadow-2xl rounded-2xl overflow-hidden relative p-10 z-10 border-8 border-black transition-transform duration-500 ease-in-out transform animate-floating">
        ...
        <div 
          ...
          style={{ backgroundImage: isLogin ? "url('https://i.postimg.cc/jq36qbCS/log2.png')" : "url('https://i.postimg.cc/26fcwfnD/Register-Img.png')" }}
        >
        </div>
        
        {/* Form Side */}
        <div className="w-full md:w-1/2 p-10">
          <h2 className="text-3xl font-bold text-center mb-6">{isLogin ? "Login Here" : "Create Account"}</h2>
          <p className="text-center text-gray-600 mb-6">Welcome to Wirely</p>
          
          <div className="flex justify-center mb-4">
            <button
              className={`px-6 py-2 rounded-lg text-lg font-semibold transition-all ${
                isLogin ? "bg-[#fbc800] text-white" : "bg-gray-200"
              }`}
  ...
  <span className="text-gray-600 text-lg font-medium mr-3">Select Role:</span>
  <div className="flex bg-gray-200 p-1 rounded-full">
    <button
      className={`px-6 py-2 rounded-full text-lg font-semibold transition-all ${
        role === "student" ? "bg-[#fbc800] text-white" : "text-gray-700"
      }`}
    ...
    <button
      className={`px-6 py-2 rounded-full text-lg font-semibold transition-all ${
        role === "teacher" ? "bg-[#fbc800] text-white" : "text-gray-700"
      }`}
      ...
      User
          ...
          {isLogin ? (
            ...
            <div>
              ...
              <div className="text-center mt-4 text-gray-600">Already have an account? <span className=" cursor-pointer" onClick={() => setIsLogin(true)}>Login</span></div>
            </div>
          )}
        </div>
      </div>
